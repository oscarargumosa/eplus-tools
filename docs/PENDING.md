...
**Ámbitos de listado + autoría (2026-09-22):** la pestaña separa ahora **Mis visiones** (las mías, borradores incluidos) · **De mi entidad** (las mías + las publicadas por compañeros con el mismo `entity_oid`) · **Publicadas** (tablón abierto de todas las públicas, con buscador). Backend: `GET /vision/public` (optionalAuth, estaba comentada) y `GET /vision/entity` nuevas; `GET /vision` pasa a devolver `{rows, entity}`. `myEntityOid()` resuelve mi entidad vía `users.organization_id → organizations.oid`, con fallback al último `entity_oid` que yo mismo vinculé. Toda visión compartida se firma con **logo + nombre de la entidad** (`entityCards()` contra `entities`+`entity_enrichment` con LEFT JOIN). Ficha pública de solo lectura (`renderPublicBrief`) para visiones ajenas. **Decisión abierta:** los borradores privados de un compañero de entidad NO se muestran — falta confirmar con Óscar si dentro de una misma ONG deberían verse.
